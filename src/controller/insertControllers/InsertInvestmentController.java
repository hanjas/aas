 ...
 * @author USER
 */
public class InsertInvestmentController implements Initializable {
    ...
    private void setInvestment() {
        try {
            ...
            investmentNameField.setItems(db.getInvestmentNames());
            TextFields.bindAutoCompletion(investmentNameField.getEditor(), db.getInvestmentNames());
            db.close();
        } catch (Exception ex) {
    ...
    private boolean checkFields() {
        if(categoryField.getText().equals("") || investmentNameField.getEditor().getText().equals(""))
            return false;
    ...
    private void save() throws Exception{
        SingleInvestmentDetails investment = new SingleInvestmentDetails(investmentNameField.getEditor().getText().replaceAll("\\s+", "_"));
        investment.insertInvestmentDetails(getDate(), categoryField.getText(), investment.getLastBalance( getDate() ), toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""), getBalance(investment.getLastBalance( getDate() ) ) );
        investment.close();
    }
    ...
    @FXML private void nextAction(KeyEvent e) throws Exception {
        if(e.getCode() == KeyCode.ENTER) {
            if(checkFields()) {
                save();
                investmentNameField.getEditor().clear();
                investmentNameField.getSelectionModel().clearSelection();
                categoryField.clear();
                toGetField.clear();
                toGiveField.clear();
            }
            investmentNameField.getEditor().requestFocus();
        }
    }
    ...
    @FXML ComboBox investmentNameField;
