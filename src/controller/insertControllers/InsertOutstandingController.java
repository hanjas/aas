 ...
 * @author BCz
 */
public class InsertOutstandingController implements Initializable {
    ...
    private void setOutstanders() {
        try {
            DBAccess db = new DBAccess();
            outstandingNameField.setItems(db.getOutstandingsNames());
            TextFields.bindAutoCompletion(outstandingNameField.getEditor(), db.getOutstandingsNames());
            db.close();
        } catch (Exception ex) {
    ...
    private void save() throws Exception{
        SingleOutstandingDetails outstanding = new SingleOutstandingDetails(outstandingNameField.getEditor().getText().replaceAll("\\s+", "_"));
        outstanding.insertOutstandingDetails(getDate(), categoryField.getText(), outstanding.getLastBalance( getDate() ), toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""), getBalance(outstanding.getLastBalance( getDate() ) ) );
        outstanding.close();
    }
    private boolean checkFields() {
        if(categoryField.getText().equals("") || outstandingNameField.getEditor().getText().equals(""))
            return false;
    ...
    @FXML private void nextAction(KeyEvent e) throws Exception {
        if(e.getCode() == KeyCode.ENTER) {
            if(checkFields()) {
                save();
                outstandingNameField.getEditor().clear();
                outstandingNameField.getSelectionModel().clearSelection();
                categoryField.clear();
                toGetField.clear();
                toGiveField.clear();
            }
            outstandingNameField.getEditor().requestFocus();
        }
    }
