...
import backend.SingleInvestmentDetails;
 ...
 * @author USER
 */
public class CreateInvestmentController implements Initializable {
    ...
    private void save() {
        try {
            SingleInvestmentDetails investment = new SingleInvestmentDetails(nameField.getText().toUpperCase().replaceAll("\\s+", "_"));
            db.createInvestment(nameField.getText().toUpperCase(), toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""), groupeField.getText());
            db.createNameInBalanceSheet("", nameField.getText().toUpperCase(), "Investment", "4", toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""));
            db.insertInvestmentDetails(nameField.getText().toUpperCase(), toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""));
            db.close();
            if(!toGetField.getText().replaceAll("," , "").equals("") && !toGiveField.getText().replaceAll("," , "").equals(""))
                investment.insertInvestmentDetails("", "", "", "", "", (Integer.parseInt(toGiveField.getText().replaceAll("," , ""))-Integer.parseInt(toGetField.getText().replaceAll("," , "")))+"");
            else if(!toGetField.getText().replaceAll("," , "").equals(""))
                investment.insertInvestmentDetails("", "", "", "", "", "-"+toGetField.getText().replaceAll("," , ""));
            else if(!toGiveField.getText().replaceAll("," , "").equals(""))
                investment.insertInvestmentDetails("", "", "", "", "", toGiveField.getText().replaceAll("," , ""));
            investment.close();
            showCreatedMsg();
        } catch (Exception ex) {
            showErrorMsg();
