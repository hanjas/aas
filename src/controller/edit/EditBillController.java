...
public class EditBillController implements Initializable {
    ...
    @FXML private void showBtnAction() throws Exception {
        ...
        List<EditBillListView> arrayList = new ArrayList<>();
        while(rs.next()) {
            EditBillListView items = new EditBillListView(rs.getString("billNo"), rs.getString("name"), rs.getString("totalBillAmt"), rs.getString("cashPaid"), stage);
            arrayList.add(items);
        }
        ObservableList<EditBillListView> myObservableList = FXCollections.observableList(arrayList);
    ...
    @FXML private ListView<EditBillListView> listView;
    private Stage stage;
