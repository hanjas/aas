 ...
 * @author USER
 */
public class EditInvestmentDeptDataController implements Initializable {
    ...
    private void setInvestmentDeptNames() {
        try {
            list.clear();
            DBAccess db = new DBAccess();
            list = db.getInvestmentDeptNames();
            investmentDeptNameField.setItems(list);
            TextFields.bindAutoCompletion(investmentDeptNameField.getEditor(), list);
            db.close();
    ...
    @FXML private void showBtnAction() {
        try {
            DBAccess db = new DBAccess();
            ResultSet rs = db.getInvestmentDeptDetails(investmentDeptNameField.getValue().toString(), getFromDate(), getToDate());
                ...
                if(!rs.getString("toget").equals("")) {
                    ...
                    listBtn.button.setOnAction(e->{
                        try {
                            ...
                            dbs.close();
                            SingleInvestmentDeptDetails so = new SingleInvestmentDeptDetails(investmentDeptNameField.getValue().toString().replaceAll("\\s+", "_"));
                            so.deleteInvestmentDeptData(listBtn.getDate(), listBtn.getIdd());
                            so.close();
                            list.remove(listBtn);
