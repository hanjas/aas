 ...
 * @author USER
 */
public class EditInvestmentDataController implements Initializable {
    ...
    private void setInvestmentNames() {
        try {
            list.clear();
            DBAccess db = new DBAccess();
            list = db.getInvestmentNames();
            investmentNameField.setItems(list);
            TextFields.bindAutoCompletion(investmentNameField.getEditor(), list);
            db.close();
    ...
    @FXML private void showBtnAction() {
        try {
            DBAccess db = new DBAccess();
            ResultSet rs = db.getInvestmentDetails(investmentNameField.getValue().toString(), getFromDate(), getToDate());
                ...
                if(!rs.getString("toget").equals("")) {
                    ...
                    listBtn.button.setOnAction(e->{
                        try {
                            ...
                            dbs.close();
                            SingleInvestmentDetails so = new SingleInvestmentDetails(investmentNameField.getValue().toString().replaceAll("\\s+", "_"));
                            so.deleteInvestmentData(listBtn.getDate(), listBtn.getIdd());
                            so.close();
                            list.remove(listBtn);
