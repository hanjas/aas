...
public class EditBuyerController implements Initializable {
    ...
    public void filterItems(String searchName) {
        ...
        try {
            ...
            for(Object name : filteredData) {
                ListWithDeleteAndRemoveBtn listBtn = new ListWithDeleteAndRemoveBtn(name.toString(), "buyer", this);
    ...
    @FXML public void createBuyer() {
        Stage buyer = new Stage();
        buyer.setTitle("Create Buyer");
        buyer.getIcons().add(new Image("/image/aas.png"));
        try{
            Pane myPane = (Pane)FXMLLoader.load(getClass().getResource("/view/CreateBuyer.fxml"));
            Scene myScene = new Scene(myPane);
            buyer.setScene(myScene);
            buyer.setMaximized(false);
            buyer.setMinWidth(400);
            buyer.setMinHeight(320);
            buyer.initOwner(myStage);
            buyer.showAndWait();
            refreshItems();
            setItems();
        }catch (Exception e) { System.out.println(e); }
    }
    @FXML public void createBuyerOnKeyPress(KeyEvent e) {
