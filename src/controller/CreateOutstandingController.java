 ...
 * @author Haxx
 */
public class CreateOutstandingController implements Initializable {
    ...
    private void save() {
        try {
            SingleOutstandingDetails outstanding = new SingleOutstandingDetails(nameField.getText().toUpperCase().replaceAll("\\s+", "_"));
            db.createOutstanding(nameField.getText().toUpperCase(), toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""), groupeField.getText());
            db.createNameInBalanceSheet("", nameField.getText().toUpperCase(), "Outstanding", "3", toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""));
            db.insertOutstanderDetails(nameField.getText().toUpperCase(), toGetField.getText().replaceAll("," , ""), toGiveField.getText().replaceAll("," , ""));
            db.close();
            if(!toGetField.getText().replaceAll("," , "").equals("") && !toGiveField.getText().replaceAll("," , "").equals(""))
                outstanding.insertOutstandingDetails("", "", "", "", "", (Integer.parseInt(toGiveField.getText().replaceAll("," , ""))-Integer.parseInt(toGetField.getText().replaceAll("," , "")))+"");
            else if(!toGetField.getText().replaceAll("," , "").equals(""))
                outstanding.insertOutstandingDetails("", "", "", "", "", "-"+toGetField.getText().replaceAll("," , ""));
            else if(!toGiveField.getText().replaceAll("," , "").equals(""))
                outstanding.insertOutstandingDetails("", "", "", "", "", toGiveField.getText().replaceAll("," , ""));
            outstanding.close();
            showCreatedMsg();
        } catch (Exception ex) {
            showErrorMsg();
